...
export default function PayGradesPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const res = await fetchPayGrades({
      ...
      setError(err instanceof Error ? err.message : "Failed to load pay grades.");
  ...
  return (
    <SuperAdminShell>
      <PageHeader
        title="Pay Grades"
        description="Manage salary grade bands, midpoints, and step increments."
      />
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] border-collapse text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                {grades.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-5 py-8 text-center text-sm text-slate-500">No pay grades found.</td>
                  </tr>
                ) : (
                  grades.map((g) => (
                    <tr key={g.id} className="transition duration-200 hover:bg-slate-900/80">
                      <td className="px-5 py-4">
                        <p className="text-white font-semibold">{g.grade}</p>
                      </td>
                      <td className="px-5 py-4 text-slate-400 font-mono text-xs">{formatCurrency(g.minSalary)}</td>
                      <td className="px-5 py-4 text-slate-400 font-mono text-xs">{formatCurrency(g.midPoint)}</td>
                      <td className="px-5 py-4 text-slate-400 font-mono text-xs">{formatCurrency(g.maxSalary)}</td>
                      <td className="px-5 py-4 text-slate-400 font-mono text-xs">{formatCurrency(g.stepIncrement)}</td>
                      <td className="px-5 py-4 text-slate-300">{g.employeeCount}</td>
                    </tr>
                  ))
                )}
              </tbody>
