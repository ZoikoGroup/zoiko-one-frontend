...
import {
  ...
  type AssetAllocationRecord, type AssetAllocationStatus,
} from "../../../lib/workforce-api";

export default function AllocationPage() {
  const [records, setRecords] = useState<AssetAllocationRecord[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      const res = await fetchAllocationRecords({
  ...
  return (
    <SuperAdminShell>
      <PageHeader
        ...
        description="Track asset allocation to employees."
      />
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            type="text" placeholder="Search by employee, asset, department..."
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[850px] border-collapse text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                {records.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-5 py-8 text-center text-sm text-slate-500">No allocation records found.</td>
                  </tr>
                ) : (
                  records.map((r) => (
                    <tr key={r.id} className="transition duration-200 hover:bg-slate-900/80">
                      ...
                      <td className="px-5 py-4 text-slate-400">{r.asset}</td>
                      <td className="px-5 py-4 text-slate-400">{r.department}</td>
                      <td className="px-5 py-4 text-slate-400">{r.allocatedDate}</td>
                      <td className="px-5 py-4 text-slate-400">{r.returnDate || "—"}</td>
