    ...
    <div className="flex h-full min-h-screen bg-slate-50 font-sans relative">
      ...
      <aside className="w-56 flex-shrink-0 border-r border-slate-200 bg-white px-3 py-6 hidden xl:flex flex-col gap-1">
        ...
        <button
          ...
          className="mt-auto flex items-center gap-2.5 rounded-2xl px-3 py-2.5 text-sm text-violet-600 hover:bg-violet-50 border border-transparent hover:border-violet-200 transition-all duration-150 font-medium"
        >
          <Sparkles size={15} />
          AI Assistant
        </button>
            ...
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400">Payroll explainer</p>
                </div>
              </div>
              <button
                onClick={() => setAiOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              <div className="rounded-2xl bg-violet-50 border border-violet-100 p-4">
                ...
                <p className="text-xs text-violet-600 leading-relaxed">
                  I can explain payroll calculations, clarify deductions, summarize
                  run statuses, and highlight anomalies.
                </p>
              </div>
              {[
              ...
              <div className="rounded-2xl bg-amber-50 border border-amber-100 p-4">
                ...
                <p className="text-xs text-amber-600 leading-relaxed">
                  AI cannot calculate payroll, change salaries, modify tax rules,
                  approve payroll, or release payments.
                </p>
              </div>
            </div>
      ...
      <div className="fixed bottom-5 right-5 z-[9999] flex flex-col gap-2 max-w-sm w-full">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`rounded-2xl border px-4 py-3 shadow-lg flex items-center justify-between text-sm transition-all duration-300 transform translate-y-0 ${
              toast.type === "success"
          ...
          >
            ...
            <button
              ...
              className="ml-3 rounded-lg p-1 hover:bg-slate-200/50 text-slate-400 hover:text-slate-600"
            >
              <X size={14} />
            </button>
          </div>
