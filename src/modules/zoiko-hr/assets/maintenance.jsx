...
const RESOURCE = "asset-maintenance";
...
export default function Maintenance() {
  ...
  const validate = (d) => {
    const e = {};
    if (!(d.assetName || d.asset_name || "").trim()) e.assetName = "Asset name is required";
    if (!(d.assetTag || d.asset_tag || "").trim()) e.assetTag = "Asset tag is required";
    if (!(d.issue || d.description || "").trim()) e.issue = "Issue description is required";
    if (!(d.reportedBy || d.reported_by || "").trim()) e.reportedBy = "Reporter name is required";
    return e;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createRecord(RESOURCE, {
        asset_name: form.assetName.trim(), asset_tag: form.assetTag.trim(),
        issue: form.issue.trim(), priority: form.priority,
        reported_by: form.reportedBy.trim(), status: "reported",
  ...
  const handleResolve = async (e) => {
    ...
    setSubmitting(true);
    try {
      await updateRecord(RESOURCE, resolveId, {
    ...
    return (
      <div className="p-6">
        <div className="flex justify-center items-center py-20">
          ...
          <span className="ml-3 text-gray-500">Loading maintenance records...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      {filtered.length === 0 && !loading ? (
        <div className="text-center py-16 bg-white rounded-xl border border-gray-200">
          <p className="text-gray-500 font-medium">{records.length === 0 ? "No maintenance records yet." : "No records match your search."}</p>
        </div>
      ) : (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={form.assetName} onChange={(e) => setForm({ ...form, assetName: e.target.value })}
                    className={`w-full border ${formErrors.assetName ? "border-red-300" : "border-gray-200"} rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500`} />
