...
import { getComplianceReport } from "../../../service/hrService";
...
const TYPE_ICONS = { PDF: FileText, XLSX: FileSpreadsheet };
...
export default function ComplianceReports() {
  ...
  useEffect(() => {
    let mounted = true;
    getComplianceReport()
      .then((res) => { if (mounted) setReports(Array.isArray(res) ? res : res?.data || []); })
      .catch(() => {})
  ...
  return (
    <HRPage title="Compliance Reports" subtitle="Generate and download compliance reports">
      <SubNav />
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 max-w-sm">
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
            <input className="w-full rounded-xl border border-gray-300 pl-9 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20" placeholder="Search reports..." value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
