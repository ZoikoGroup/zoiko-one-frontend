...
export default function PerformanceSettings() {
  ...
  return (
    <HRPage title="Performance Settings" subtitle="Configure performance management preferences">
      <SubNav />
      <div className="space-y-6">
        ...
        <div className="flex gap-1 border-b border-gray-200">
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === tab.id ? "border-blue-500 text-blue-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <select className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                  <option>Quarterly</option><option>Bi-Annual</option><option>Annual</option>
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Default Reviewer</label>
                <select className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                  <option>Direct Manager</option><option>Skip-Level Manager</option><option>HR Department</option>
                </select>
              </div>
              <div>
              ...
              <div>
                ...
                <select className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                  <option>Yes - Always</option><option>Optional</option><option>No</option>
                </select>
              </div>
            </div>
          </div>
                  ...
                  {reviewCycles.map((cycle) => (
                    <tr key={cycle.id} className="border-b border-gray-100">
                      ...
                      <td className="py-3 px-4">
                        <span className={`text-xs font-medium px-2 py-1 rounded-full ${cycle.status === "completed" ? "bg-green-100 text-green-700" : cycle.status === "active" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-600"}`}>{cycle.status}</span>
                      </td>
                      <td className="py-3 px-4"><button className="text-blue-600 hover:text-blue-800 text-xs font-medium">Configure</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              {ratingScales.map((r) => (
                <div key={r.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                  <div className="flex items-center gap-3">
                    <Star className={`w-4 h-4 ${parseInt(r.id) <= 3 ? "text-yellow-400" : parseInt(r.id) <= 4 ? "text-blue-400" : "text-green-500"}`} />
                    <span className="text-sm text-gray-900">{r.label}</span>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" defaultChecked={r.active} className="sr-only peer" />
        ...
        {activeTab === "kpi" && (
          <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
            ...
            {["Allow employees to create personal KPIs", "Manager approval required for KPI changes", "Auto-calculate weighted scores", "Show KPI progress on dashboard", "Enable peer review for KPI assessment"].map((item, idx) => (
        ...
        {activeTab === "notifications" && (
          <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
            ...
            {["Review cycle starts", "Review deadline approaching", "New feedback received", "Goal status changes", "360 review invitation", "Appraisal scheduled"].map((item) => (
