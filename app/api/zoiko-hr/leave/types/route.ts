...
import { listLeaveTypes, createLeaveType } from "@/app/services/leaveService";
...
export const GET = withPermission("workforce.*", async function GET(request: NextRequest) {
  ...
  const result = await listLeaveTypes({ filters, skip, take, orderBy, orderDir });
...
export const POST = withPermission("workforce.*", async function POST(request: NextRequest) {
  ...
  const leaveType = await createLeaveType({
    ...
    category: body.category ?? "PAID",
    maxDaysPerYear: body.maxDaysPerYear,
    minDaysRequired: body.minDaysRequired,
    requiresApproval: body.requiresApproval,
    requiresMedicalCert: body.requiresMedicalCert,
    attachmentRequired: body.attachmentRequired,
  });

  return Response.json({ data: leaveType }, { status: 201 });
});
