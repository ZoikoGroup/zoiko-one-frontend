...
export default function ZoikoHRDeductions() {
    ...
    if (search.trim()) {
      ...
      result = result.filter(
        (i) =>
          ...
          (i.deduction_type && i.deduction_type.toLowerCase().includes(q)) ||
  ...
  const validateForm = (data) => {
    ...
    if (!data.deduction_type) errors.deduction_type = "Deduction type is required";
  ...
  const handleCreate = async (e) => {
    ...
    try {
      await createDeduction({
        ...
        deduction_type: formData.deduction_type.trim() || null,
      ...
      setFormErrors({ submit: err.message || "Failed to create deduction" });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      if (Object.keys(payload).length > 0) {
        await updateDeduction(editItem.id, payload);
      }
      setShowEditModal(false);
      setEditItem(null);
      await fetchItems();
    } catch (err) {
      setFormErrors({ submit: err.message || "Failed to update deduction" });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setError(err.message || "Failed to delete deduction");
    }
  };

  if (loading && items.length === 0) {
    return (
      <HRPage title="Deductions" subtitle="Manage employee deductions and contributions.">
        <div className="flex justify-center items-center py-20">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
          <span className="ml-3 text-gray-500">Loading deductions...</span>
        </div>
      </HRPage>
    );
  }
    ...
    <HRPage title="Deductions" subtitle="Manage employee deductions and contributions.">
        ...
        <div className="flex flex-wrap justify-between items-center gap-4">
          <div className="flex flex-wrap gap-3">
            ...
            <div className="bg-white px-4 py-2 border border-red-100 rounded-lg shadow-sm text-sm">
              ...
              <span className="font-bold text-red-600">${stats.monthlyTotal.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
            </div>
          </div>
          <button
        ...
        {filtered.length === 0 && !loading ? (
          <div className="text-center py-16 bg-white rounded-xl border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-500 font-medium">
              {items.length === 0
                ? "No deductions yet. Add your first deduction to get started."
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 border-b border-gray-100">
                    <tr>
                      ...
                      <th className="text-left px-4 py-3 font-semibold text-gray-600">Mandatory</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {paginated.map((item) => (
                      <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-gray-700">{item.deduction_type || <span className="text-gray-300">-</span>}</td>
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${item.is_mandatory ? "bg-red-100 text-red-800" : "bg-gray-100 text-gray-800"}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h2 className="text-lg font-bold text-gray-800">Add Deduction</h2>
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full border ${formErrors.deduction_type ? "border-red-300" : "border-gray-200"} rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500`}
                />
                {formErrors.deduction_type && <p className="text-red-500 text-xs mt-1">{formErrors.deduction_type}</p>}
              </div>
              <div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" disabled={submitting} className="px-4 py-2 text-sm bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white rounded-lg font-medium transition-colors">
                  {submitting ? "Creating..." : "Create Deduction"}
                </button>
              </div>
            </form>
          </div>
      ...
      {showEditModal && editItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h2 className="text-lg font-bold text-gray-800">Update Deduction</h2>
            ...
            <form onSubmit={handleUpdate} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditForm({ ...editForm, deduction_type: e.target.value })}
                  className={`w-full border ${formErrors.deduction_type ? "border-red-300" : "border-gray-200"} rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500`}
                />
                {formErrors.deduction_type && <p className="text-red-500 text-xs mt-1">{formErrors.deduction_type}</p>}
              </div>
