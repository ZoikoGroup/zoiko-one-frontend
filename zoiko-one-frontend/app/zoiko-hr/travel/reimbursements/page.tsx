  ...
  return (
    <SuperAdminShell>
      <PageHeader
        ...
        description="Track and manage employee expense reimbursements."
      />
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" placeholder="Search by employee or category..." value={search}
      ...
      {loading ? (
        ...
        <section className="overflow-hidden rounded-[28px] border border-slate-800 bg-[#0b1220] shadow-[0_20px_80px_rgba(0,0,0,0.35)]">
          <div className="border-b border-slate-800 px-5 py-4">
            <h2 className="text-lg font-semibold text-white">Reimbursements <span className="ml-2 text-sm font-normal text-slate-400">({total})</span></h2>
          </div>
            ...
            <table className="w-full min-w-[1000px] border-collapse text-left text-sm">
              <thead className="bg-slate-950 text-xs uppercase text-slate-500">
                <tr>
                  ...
                  <th className="px-5 py-3 font-semibold">Processed Date</th>
              ...
              <tbody className="divide-y divide-slate-800">
                {reimbursements.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-5 py-8 text-center text-sm text-slate-500">No reimbursements found.</td>
                  </tr>
                ) : (
                  reimbursements.map((r) => (
                    <tr key={r.id} className="transition duration-200 hover:bg-slate-900/80">
                      ...
                      <td className="px-5 py-4 text-slate-300 font-medium">{formatCurrency(r.amount)}</td>
                      <td className="px-5 py-4 text-slate-400">{r.paymentMethod}</td>
                      <td className="px-5 py-4 text-slate-400">{r.processedDate}</td>
