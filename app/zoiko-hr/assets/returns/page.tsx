...
export default function ReturnsPage() {
  const [records, setRecords] = useState<AssetReturnRecord[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      const res = await fetchReturnRecords({
      ...
      setError(err instanceof Error ? err.message : "Failed to load return records.");
  ...
  return (
    <SuperAdminShell>
      <PageHeader
        title="Asset Returns"
        description="Track returned assets and their condition."
      />
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            type="text" placeholder="Search by asset, employee, condition..."
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[800px] border-collapse text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                {records.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-5 py-8 text-center text-sm text-slate-500">No return records found.</td>
                  </tr>
                ) : (
                  records.map((r) => (
                    <tr key={r.id} className="transition duration-200 hover:bg-slate-900/80">
                      ...
                      <td className="px-5 py-4 text-slate-400">{r.employee}</td>
                      <td className="px-5 py-4 text-slate-400">{r.returnedDate}</td>
                      <td className="px-5 py-4">
                        <span className={`font-semibold ${CONDITION_COLORS[r.condition] ?? "text-slate-400"}`}>{r.condition}</span>
                      </td>
                      <td className="px-5 py-4 text-slate-400 max-w-[250px] truncate">{r.remarks}</td>
                    </tr>
                  ))
                )}
              </tbody>
