...
import {
  fetchCourses,
  type Course, type CourseLevel, type CourseStatus,
} from "../../../lib/workforce-api";

const CATEGORIES = ["All", "Management", "Engineering", "Data", "Design", "Marketing", "Soft Skills", "Security", "Wellness"];
...
export default function CoursesPage() {
  ...
  useEffect(() => { loadData(); }, [search, categoryFilter, levelFilter, statusFilter, page]);
  ...
  return (
    <SuperAdminShell>
      <PageHeader
        title="Courses"
        description="Browse and manage all learning courses."
      />
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" placeholder="Search by name, instructor, or category..." value={search}
      ...
      {loading ? (
        ...
        <section className="overflow-hidden rounded-[28px] border border-slate-800 bg-[#0b1220] shadow-[0_20px_80px_rgba(0,0,0,0.35)]">
          <div className="border-b border-slate-800 px-5 py-4">
            <h2 className="text-lg font-semibold text-white">Courses <span className="ml-2 text-sm font-normal text-slate-400">({total})</span></h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] border-collapse text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                {courses.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-5 py-8 text-center text-sm text-slate-500">No courses found.</td>
                  </tr>
                ) : (
                  courses.map((c) => (
                    <tr key={c.id} className="transition duration-200 hover:bg-slate-900/80">
                      ...
                      <td className="px-5 py-4 text-slate-400">{c.instructor}</td>
                      <td className="px-5 py-4 text-slate-400">{c.duration}</td>
                      <td className="px-5 py-4"><StatusBadge status={c.level} /></td>
                      <td className="px-5 py-4 text-slate-300">{c.enrolledCount}</td>
