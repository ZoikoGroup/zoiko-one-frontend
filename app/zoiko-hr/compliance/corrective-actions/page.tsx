...
import {
  fetchCorrectiveActions, createCorrectiveAction, updateCorrectiveAction,
  type CorrectiveAction, type CorrectiveActionPriority, type CorrectiveActionStatus,
} from "../../../lib/workforce-api";

const PRIORITIES: CorrectiveActionPriority[] = ["CRITICAL", "HIGH", "MEDIUM", "LOW"];
const STATUSES: CorrectiveActionStatus[] = ["OPEN", "IN_PROGRESS", "COMPLETED", "VERIFIED", "CLOSED"];
...
export default function CorrectiveActionsPage() {
  ...
  const [formData, setFormData] = useState({ title: "", description: "", assignedTo: "", priority: "MEDIUM" as CorrectiveActionPriority, dueDate: "" });
  const [submitting, setSubmitting] = useState(false);
  const [formError, setFormError] = useState("");

  const loadData = async () => {
    try {
      const res = await fetchCorrectiveActions({
  ...
  useEffect(() => {
    ...
    fetchCorrectiveActions({
      ...
      .then((res) => { if (!cancelled) { setActions(res.data); setTotal(res.total); setError(""); setLoading(false); } })
      .catch((err) => { if (!cancelled) { setError(err instanceof Error ? err.message : "Failed to load corrective actions."); setLoading(false); } });
    return () => { cancelled = true; };
  }, [search, statusFilter, priorityFilter, page]);

  const openCreate = () => {
    setEditId(null); setFormData({ title: "", description: "", assignedTo: "", priority: "MEDIUM", dueDate: "" }); setFormError(""); setShowForm(true);
  };

  const openEdit = (a: CorrectiveAction) => {
    setEditId(a.id); setFormData({ title: a.title, description: a.description ?? "", assignedTo: a.assignedTo, priority: a.priority, dueDate: a.dueDate ? a.dueDate.split("T")[0] : "" }); setFormError(""); setShowForm(true);
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.title.trim() || !formData.assignedTo.trim()) { setFormError("Title and assignee are required."); return; }
    setSubmitting(true); setFormError("");
    try {
      if (editId) {
        await updateCorrectiveAction(editId, { title: formData.title, description: formData.description || undefined, assignedTo: formData.assignedTo, priority: formData.priority, dueDate: formData.dueDate || undefined });
        setToast("Action updated.");
      } else {
        await createCorrectiveAction({ title: formData.title, description: formData.description || undefined, assignedTo: formData.assignedTo, priority: formData.priority, dueDate: formData.dueDate || undefined });
  ...
  return (
    <SuperAdminShell>
      <PageHeader
        ...
        description="Manage and track corrective actions for compliance violations and audit findings."
        action={
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" placeholder="Search by title or assignee..." value={search}
      ...
      {loading ? (
        ...
        <section className="overflow-hidden rounded-[28px] border border-slate-800 bg-[#0b1220] shadow-[0_20px_80px_rgba(0,0,0,0.35)]">
          <div className="border-b border-slate-800 px-5 py-4">
            <h2 className="text-lg font-semibold text-white">Corrective Actions <span className="ml-2 text-sm font-normal text-slate-400">({total})</span></h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1000px] border-collapse text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                {actions.length === 0 ? (
                  <tr><td colSpan={6} className="px-5 py-8 text-center text-sm text-slate-500">No corrective actions found.</td></tr>
                ) : (
                  actions.map((a) => (
                    <tr key={a.id} className="transition duration-200 hover:bg-slate-900/80">
                      ...
                      <td className="px-5 py-4 text-slate-400">{a.dueDate ? new Date(a.dueDate).toLocaleDateString() : "-"}</td>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4" onClick={() => setShowForm(false)}>
          <div className="w-full max-w-lg rounded-[28px] border border-slate-800 bg-[#0b1220] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.5)]" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-white">{editId ? "Edit Action" : "Create Corrective Action"}</h3>
            <form onSubmit={handleSubmit} className="mt-5 space-y-4">
              ...
              <div>
                ...
                <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                  className="w-full rounded-3xl border border-slate-800 bg-slate-950 px-4 py-2.5 text-sm text-white outline-none focus:border-indigo-500" placeholder="e.g. Update data retention policy" />
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-400">Assigned To</label>
                <input type="text" value={formData.assignedTo} onChange={(e) => setFormData({ ...formData, assignedTo: e.target.value })}
                  className="w-full rounded-3xl border border-slate-800 bg-slate-950 px-4 py-2.5 text-sm text-white outline-none focus:border-indigo-500" placeholder="Employee name" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-400">Priority</label>
                  <select value={formData.priority} onChange={(e) => setFormData({ ...formData, priority: e.target.value as CorrectiveActionPriority })}
              ...
              <div>
                ...
                <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                  className="w-full rounded-3xl border border-slate-800 bg-slate-950 px-4 py-2.5 text-sm text-white outline-none focus:border-indigo-500" rows={3} placeholder="Description of the corrective action..." />
              </div>
