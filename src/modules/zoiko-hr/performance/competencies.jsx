...
const competencyData = [
  ...
  { category: "Domain Knowledge", items: [
...
function StatusBadge({ status }) {
  const m = { high: "bg-red-100 text-red-800", medium: "bg-yellow-100 text-yellow-800", low: "bg-green-100 text-green-800" };
...
export default function Competencies() {
  ...
  const getLevelColor = (level) => {
    const m = { Expert: "bg-purple-100 text-purple-700", Advanced: "bg-blue-100 text-blue-700", Intermediate: "bg-yellow-100 text-yellow-700", Beginner: "bg-gray-100 text-gray-700" };
  ...
  return (
    <HRPage title="Competencies" subtitle="Skill matrix and competency tracking">
      <SubNav />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div key={category.category} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                ...
                {expanded.includes(idx) && (
                  <div className="divide-y divide-gray-100">
                    {category.items.filter((item) => !search || item.name.toLowerCase().includes(search.toLowerCase())).map((item) => (
                      <div key={item.name} className="flex items-center justify-between px-5 py-3">
                        <div className="flex-1">
                          <p className="text-sm font-medium text-gray-900">{item.name}</p>
                          <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${getLevelColor(item.level)}`}>{item.level}</span>
                        </div>
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <h3 className="font-semibold text-gray-900 mb-4 flex items-center gap-2"><Users className="w-4 h-4 text-blue-500" /> Team Scores</h3>
              <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2"><TrendingUp className="w-4 h-4 text-blue-500" /> Recommended Training</h3>
              <div className="space-y-3">
                {[
                  ...
                  <div key={t.name} className="flex items-start gap-2 p-2 rounded-lg hover:bg-gray-50">
                    <BookOpen className="w-4 h-4 text-blue-500 mt-0.5" />
                    <div>
                      <p className="text-sm font-medium text-gray-900">{t.name}</p>
                      <p className="text-xs text-gray-500">Addresses: {t.gap}</p>
